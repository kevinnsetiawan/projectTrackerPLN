        ...
        <nav className="px-3 py-4 space-y-1">
          {NAV.filter((n) => !n.adminOnly || can('admin')).map(({ to, label, icon: Icon, end }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition ${isActive ? 'bg-white/15 text-white shadow-inner' : 'text-white/60 hover:bg-white/5 hover:text-white'
                }`
              }
            >
